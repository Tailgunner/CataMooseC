...
is almost totally backwards compatible with 5.xx, but some functionality
has changed in backwards incompatible ways. Other parts have been
...
version 6.00. This page is primarily for someone wanting to know
exactly what changed.

If you want to make sure that a script which ran with 5.xx will run
with 6.xx, refer to the Date::Manip::Migration5to6 document.
...
=item B<Minimal time zone support>

Time zone support in 5.xx was broken. Determining a time zone, and
understanding daylight saving time changes was minimal and was
missing a lot of necessary functionality.
...
Date::Manip to run on older versions of perl.  Prior to 5.10, none of
the new features would have had a major impact on how Date::Manip was
written, however that changed in 5.10.
...
All of these are now deprecated. They will continue to work for a
while, but they will be removed at some point. Instead, the single
config variable:
...
=item B<TodayIsMidnight removed>

The TodayIsMidnight variable is no longer supported. The DefaultTime
...
in some future version:
   ...
   ConvTZ          Ignored.  Time zone handling is done
                   automatically, so this is no longer
                   necessary.

   Internal        Renamed to Printable (Internal still
                   works for a while)

   DeltaSigns      Ignored. Use the Date::Manip::Delta::printf
                   method to print deltas

   UpdateCurrTZ    Ignored. With real time zone handling in
...
now, 24 hours in the future, and 24 hours in the past respectively.
...
from a possible misinterpretation of the standard.
...
including the day of week number (Monday=1 to Sunday=7), nth
day of week (1 being the 1st day of the week, -1 being the last
day of the week), and others.

In Date::Manip 5.xx, the 2nd of these two definitions was used
in a few cases.
