...
Gregorian calendar (Date::Manip does NOT provide functionality for
...
that none of the other modules have.  Date::Manip is the "Swiss Army
Knife" of Date modules.
...
As of 5.55/6.00, I'm abandoning the even/odd behavior started in
5.41. Every release meant changing the version number twice. I had to
change it from the odd (development version) to the even (release
version). Then I had to package it up. Then I had to change it to the
next odd (development version). It's more error prone, and more of a
nuisance, and I've seen no benefit from it.
...
=item B<Making the primary interface OO>
...
not mentioned in the HISTORY file.  These include suggestions which
...
I keep every single suggestion I've ever received and periodically
review the unimplemented ones to see if it's something I'm interested
in, so even suggestions made years in the past may still appear in
future versions of Date::Manip, and the original requester will be
attributed at that point. For those who have sent in requests/reports
that had been previously made by someone else, thank you too.  I'd
much rather have a suggestion made twice than not at all.
